    ...
    return (
        <div className="lg:m-15 sm:m-5 lg:gap-20 lg:grid grid-cols-3">
            <div className="col-span-2">
                {data.map((item, index) => (
                    <SummaryCard key={index} item={item[1]} q={q} idx={index} change={changeQ} id={item[0]} size={item[2]} />
                ))}
            </div>
            <div className="border text-center pb-10 max-h-100 rounded">
                ...
                <Button onClick={() => {
                    ...
                    deleteItems();
                    navigate('/')
                }} variant="contained" sx={{ width: "90%", px: "2.5rem", py: ".7rem", bgcolor: "#d5" }}>Place Order</Button>
            </div>
        </div>
    )
}